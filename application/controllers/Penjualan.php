...
class Penjualan extends CI_Controller {
	...
	function submit_cart() {
	 ...
	 public function nota($id) {
		...
		$data['items'] = $this->db->query("select kode_penjualan,sum(harga_diskon) harga_diskon,harga_jual,sum(jml) jml,nama_brg from tb_penjualan tp
	...
	function load_data_riwayat() {
		...
		foreach ($list as $item) { 
			...
			if ($item->status_penjualan == 0){
				...
				$cetak = '<a href="' . site_url('penjualan/nota/' . str_replace('/', 'O', $item->kode_penjualan)) . '" target="_blank" class="badge badge-light border btn-cetak-inv">Cetak struk</a>';
